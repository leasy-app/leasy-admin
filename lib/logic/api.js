...
export function useStats() {
    const { data, error } = useSWR(`${BASE}/Stats`, async (args) => {
        let x = {
            users:0,
            posts:0,
            courses:0,
        ...
        let users = await fetcher(`${BASE}/getUser`);
        let posts = await fetcher(`${BASE}/Posts`);
        let courses = await fetcher(`${BASE}/GetCourse`);
        if(users && users[0]){
            x.users = users.length;
            users.forEach(u => {
                x.reads += u.read;
            })
        }
        if(posts && posts[0]){
            x.posts = posts.length;
        }
        if(courses && courses[0]){
            x.courses = courses.length;
        }
        return x;
    })
    return {
